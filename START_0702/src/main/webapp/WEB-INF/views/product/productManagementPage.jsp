...
<div class="main-wrap">
	...
	<div class="product-wrap">
		<div>
			<form action="productManagementSelect" style="text-align: center; padding: 10px 0 0 0;">
				<select name="column" style="height: 25px;">
					<option value="m_Id">판매자 아이디</option>
					<option value="p_Name">제품명</option>
				</select> <input type="text" name="query" style="height: 20px;"> 
				<input type="hidden" name="p_State" value="${p_State }"> 
				<input type="submit" value="검색">
			</form>
		</div>
		
		<p style="text-align: right; margin: auto; padding-right: 15%;">상품수 : ${totalRecord }</p>
			...
			<c:if test="${not empty pDTO }">
				...
				<c:forEach var="pDTO" items="${pDTO }">
					...
					<a href="productViewPage?p_No=${pDTO.p_No }"> 
						<img alt="${pDTO.p_Img_Path }" width="300px" height="300px" src="${pageContext.request.contextPath }/resources/storage/${pDTO.p_Img_Path }">
					</a>
						<div class="desc">
							<h3 class="desc-title">${pDTO.p_Name }</h3>
							<br> 판매자 : ${pDTO.m_Id } 가격 :
							<fmt:formatNumber value="${pDTO.p_Price }" />원<br><br>
							<!--  <span> -->
		...
		<div class="number-box">${pageView }</div>
	</div>
